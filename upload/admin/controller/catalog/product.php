...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
        ...
        //added
        if($filter_category){
            $categories=$this->model_catalog_category->getCategories(['filter_name' => $filter_category]);
            if(!empty($categories)) {
                $categories_id = array_column($categories, 'category_id');
                $category_product = [];
                foreach ($categories_id as $category_id) {
                    $category_products_by_id = $this->model_catalog_product->getProductsByCategoryId($category_id);
                    foreach ($category_products_by_id as $category_product_by_id) {
                        $category_product[] = $category_product_by_id['product_id'];
                    }
                }
                foreach ($results as $key => $result){
                    if(!in_array($result['product_id'], $category_product)){
                        unset($results[$key]);
                    }
                }
            }else{
        ...
        $data['entry_category_one'] = $this->language->get('entry_category_one');
